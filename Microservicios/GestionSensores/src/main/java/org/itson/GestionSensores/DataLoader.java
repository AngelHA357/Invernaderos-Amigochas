    ...
    CommandLineRunner initData(IInvernaderosRepository invernaderosRepository, IGestionSensoresRepository gestionSensoresRepository) {
        return args -> {
            ...
            if (countInvernaderos == 0) {
                ...
                gestionSensoresRepository.save(new Sensor(new ObjectId(), "SEN-A01", "AA:BB:CC:DD:EE:FF", "SensorTech", "ST-100", "Temperatura", "°C", new ObjectId(invA.get_id()), "Sector 1", "Fila A"));
                gestionSensoresRepository.save(new Sensor(new ObjectId(), "SEN-A02", "AA:BB:CC:DD:EE:01", "SensorTech", "ST-101", "Humedad", "%", new ObjectId(invA.get_id()), "Sector 2", "Fila B"));

                // Invernadero B - Sensores
                gestionSensoresRepository.save(new Sensor(new ObjectId(), "SEN-B01", "BB:CC:DD:EE:FF:01", "EcoSense", "ES-200", "Temperatura", "°C", new ObjectId(invB.get_id()), "Sector 1", "Fila A"));
                gestionSensoresRepository.save(new Sensor(new ObjectId(), "SEN-B02", "BB:CC:DD:EE:FF:02", "EcoSense", "ES-201", "Humedad", "%", new ObjectId(invB.get_id()), "Sector 2", "Fila B"));

                // Invernadero C - Sensores
                gestionSensoresRepository.save(new Sensor(new ObjectId(), "SEN-C01", "CC:DD:EE:FF:01:02", "TempTech", "T-300", "Temperatura", "°C", new ObjectId(invC.get_id()), "Sector 1", "Fila A"));
                gestionSensoresRepository.save(new Sensor(new ObjectId(), "SEN-C02", "CC:DD:EE:FF:01:03", "TempTech", "T-301", "Humedad", "%", new ObjectId(invC.get_id()), "Sector 2", "Fila B"));

                // Invernadero D - Sensores
                gestionSensoresRepository.save(new Sensor(new ObjectId(), "SEN-D01", "DD:EE:FF:01:02:03", "GreenGrow", "GG-500", "Temperatura", "°C", new ObjectId(invD.get_id()), "Sector 1", "Fila A"));
                gestionSensoresRepository.save(new Sensor(new ObjectId(), "SEN-D02", "DD:EE:FF:01:02:04", "GreenGrow", "GG-501", "Humedad", "%", new ObjectId(invD.get_id()), "Sector 2", "Fila B"));
